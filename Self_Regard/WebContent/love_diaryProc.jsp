...
<body>
	...
	<div class="joinForm_div">
		<table class="table_content" >
			...
			<tbody>
				...
				Love_DiaryDAO love_diaryDAO = new Love_DiaryDAO();
					ArrayList<Diary> list = love_diaryDAO.getList(pageNumber, userID);
					
					for(int i = 0; i < list.size(); i++){
				%>
						<tr>
							<td class="t1"><%= list.get(i).getID() %></td>
							<td class="t2"><a href="love_diaryView.jsp?ID=<%= list.get(i).getID() %>"><%= list.get(i).getTitle() %></a></td>
		...
		<%
			if(pageNumber != 1){
		%>
				<a href="love_diary.jsp?pageNumber=<%= pageNumber - 1 %>"> 이전 </a>
		<%
			} if(love_diaryDAO.nextPage(userID ,pageNumber + 1)){
		%>
			<a href="love_diary.jsp?pageNumber=<%= pageNumber + 1 %>"> 다음 </a>
		<%
			}
		%>
		
		<a href="love_diaryWrite.jsp" class="table_write"><button class="snip1535">write</button></a>
	</div>
</html>