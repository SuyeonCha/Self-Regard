...
<body>
	...
	<div class="joinForm_div">
		<table class="table_content" >
			...
			<tbody>
				<%
				Travel_DiaryDAO travel_diaryDAO = new Travel_DiaryDAO();
					ArrayList<Diary> list = travel_diaryDAO.getList(pageNumber, userID);
					
					for(int i = 0; i < list.size(); i++){
				%>
						<tr>
							<td class="t1"><%= list.get(i).getID() %></td>
							<td class="t2"><a href="travel_diaryView.jsp?ID=<%= list.get(i).getID() %>"><%= list.get(i).getTitle() %></a></td>
		...
		<%
			if(pageNumber != 1){
		%>
				<a href="travel_diary.jsp?pageNumber=<%= pageNumber - 1 %>"> 이전 </a>
		<%
			} if(travel_diaryDAO.nextPage(userID ,pageNumber + 1)){
		%>
			<a href="travel_diary.jsp?pageNumber=<%= pageNumber + 1 %>"> 다음 </a>
		<%
			}
		%>
		
		<a href="travel_diaryWrite.jsp" class="table_write"><button class="snip1535">write</button></a>
	</div>
</html>